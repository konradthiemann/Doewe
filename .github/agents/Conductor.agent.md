...
tools: ['execute/getTerminalOutput', 'execute/runInTerminal', 'read/terminalLastCommand', 'read/terminalSelection', 'execute/createAndRunTask', 'edit', 'search', 'todo', 'agent', 'search/usages', 'read/problems', 'search/changes', 'execute/testFailure', 'web/fetch']
...
<project_context>
You are working on **Doewe** – a personal finance tracking app built with:
- **Next.js 14** (App Router) + TypeScript + Prisma + Tailwind CSS
- **Monorepo** (npm workspaces): `apps/web/` (Next.js app), `packages/shared/` (domain primitives), `shared/` (ESLint/TSConfig baselines)
- **Data flow**: UI components → API routes (`app/api/*`) → Prisma client → PostgreSQL

Key conventions:
- **Money**: Always use `Cents` (integer) from `@doewe/shared` – never floating-point
- **API routes**: Export HTTP verb functions, always check auth with `getSessionUser()` first, validate with Zod
- **Testing**: Vitest with `TEST_USER_ID_BYPASS` for auth bypass in API tests
- **i18n**: Both `de.ts` and `en.ts` locales must be updated for new UI text
- **Commits**: Conventional commits (`feat:`, `fix:`, `chore:`) with Goal/Why/How body
- **After commits**: Update `CHANGELOG.md` and `monorepoTimeline.md` (newest on top)

Quality gates: `npm run lint && npm run typecheck && npm run test`
After Prisma schema changes: `npm --workspace @doewe/web run prisma:generate`
</project_context>
...
2. **Update Project Files**: Remind user to update `CHANGELOG.md` and `monorepoTimeline.md` (newest on top).
...
- Mention key Doewe files: `apps/web/prisma/schema.prisma`, `packages/shared/src/`, `apps/web/app/api/`, `apps/web/lib/`
...
- Remind them to use `Cents` for money, Zod for validation, and `getSessionUser()` for auth
...
- Remind them to check Doewe conventions: Cents for money, auth checks, Zod validation, i18n in both locales
</subagent_instructions>
