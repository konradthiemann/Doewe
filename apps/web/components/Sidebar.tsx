 ...
 * secondary links (settings, categories, tax, legal) plus the sign-out action
 * from AppChrome's drawer, reusing the same i18n keys and active-state styling
...
export default function Sidebar() {
  ...
  const secondaryLinks: Array<{ href: string; label: string; icon: JSX.Element }> = [
    ...
    // Receipt scanner — gated by NEXT_PUBLIC_RECEIPT_SCANNER_ENABLED (Issue #53)
    ...(process.env.NEXT_PUBLIC_RECEIPT_SCANNER_ENABLED === "1"
      ? [
          {
            href: "/scan",
            label: t("nav.scan"),
