...
export default function HomePage() {
  ...
  // Expenses only (without savings)
  const projectedExpenses = projectedOutcome;
  // Total allocated = expenses + savings
  const projectedTotal = projectedExpenses + totalSavingsTransfer;
  const projectedLeft = Math.max(0, projectedIncome - projectedTotal);
  const expensesPercent = projectedIncome > 0 ? Math.min(100, Math.round((projectedExpenses / projectedIncome) * 100)) : 0;
  const savedPercent = projectedIncome > 0 ? Math.min(100 - expensesPercent, Math.round((totalSavingsTransfer / projectedIncome) * 100)) : 0;
  const leftPercent = projectedIncome > 0 ? Math.max(0, 100 - expensesPercent - savedPercent) : 0;
  const overspent = projectedIncome > 0 ? Math.max(0, projectedTotal - projectedIncome) : projectedTotal;
              ...
              <figcaption id="income-usage-summary" className="space-y-3 text-sm text-gray-700 dark:text-neutral-300">
                <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label={t("dashboard.incomeReportBreakdown")}>
                  <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-slate-800 dark:border-neutral-700 dark:bg-neutral-800/70 dark:text-neutral-200">
                    ...
                    <dd className="text-lg font-semibold text-red-600 dark:text-red-400">{formatCurrency(projectedExpenses)}</dd>
                    <p className="text-xs text-slate-600 dark:text-neutral-400">
                      {t("dashboard.spentOfIncome", { percent: expensesPercent })}
                    </p>
                  </div>
                  <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-slate-800 dark:border-neutral-700 dark:bg-neutral-800/70 dark:text-neutral-200">
                    <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-neutral-400">{t("dashboard.saved")}</dt>
                    <dd className="text-lg font-semibold text-blue-600 dark:text-blue-400">{formatCurrency(totalSavingsTransfer)}</dd>
                    <p className="text-xs text-slate-600 dark:text-neutral-400">
                      {t("dashboard.savedOfIncome", { percent: savedPercent })}
                    </p>
                  </div>
                  <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-slate-800 dark:border-neutral-700 dark:bg-neutral-800/70 dark:text-neutral-200">
                ...
                <p>
                  {t("dashboard.spentSummary", {
                    spent: formatCurrency(projectedExpenses),
